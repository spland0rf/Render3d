...
//===================================
	...
	public static void drawSolidTriangle( Triangle t, int color, Material mat)
	{
		...
		if (dy_1_2 != 0) {
			...
			for (int i=v1.y; i<v2.y; i++) {
				if (i>0 && i<_height) {
					drawSolidSpan( i, lx>>16, rx>>16, lz, rz, color, mat);
				}
				lx += dx_1_3;
				rx += dx_1_2;
				lz += dz_1_3;
	...
	public static void drawSolidSpan( int y, int lx, int rx, int lz, int rz, int color, Material mat)
	{
...
//===========================
	...
	public static void drawSolidTextureTriangle( Triangle t, Material mat)
	{
		// Order vertices {v1,v2,v3} by increasing Y
		Vertex v1 = t.v1;
		Vertex v2 = t.v2;
			...
			for (int i=v1.y; i<v2.y; i++) {
				if (i>0 && i<_height) {
					if (mat.SPEED == Material.FAST16) {
						drawFast16solidTextureSpan( i, lx>>16, rx>>16, lz, rz, ls, rs, lt, rt, mat);
					} 
					else if (mat.SPEED == Material.SLOW) {
						drawSolidTextureSpan( i, lx>>16, rx>>16, lz, rz, ls, rs, lt, rt, mat);
					}
				}
				lx += dx_1_3;
				rx += dx_1_2;
 ...
 public void drawTranspTriangle( Triangle t, rMaterial mat)
    {
		...
		if (dy_1_2 != 0) {
			...
			for (int i=v1.y; i<v2.y; i++) {
				if (i>0 && i<_height) {
					transpSpan( i, lx>>16, rx>>16, lz, rz, mat);
				}
				lx += dx_1_3;
				rx += dx_1_2;
				lz += dz_1_3;
